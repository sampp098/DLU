...
void XDlu_Set_loop_r(XDlu *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XDlu_WriteReg(InstancePtr->Crtl_bus_BaseAddress, XDLU_CRTL_BUS_ADDR_LOOP_R_DATA, Data);
}

u32 XDlu_Get_loop_r(XDlu *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XDlu_ReadReg(InstancePtr->Crtl_bus_BaseAddress, XDLU_CRTL_BUS_ADDR_LOOP_R_DATA);
    return Data;
}
